...
npm install
npm run seed   # creates the following default accounts:
                # Admin:    admin@minierp.com / Admin@123
                # Manager:  manager@minierp.com / Manager@123
                # Employee: employee@minierp.com / Employee@123
npm run dev
```
...
Server runs at `/`, health check at `/health`.
...
| GET    | /api/v1/sales     | admin, manager, employee | sale history, paginated                                  |
| GET    | /api/v1/sales/:id | admin, manager, employee | populated with customer + soldBy                         |
...
> standalone local `mongodb` will throw `Transaction numbers are only allowed on a replica set member`.
